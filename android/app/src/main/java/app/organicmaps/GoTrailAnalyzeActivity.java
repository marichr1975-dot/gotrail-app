...
public class GoTrailAnalyzeActivity extends Activity implements SearchListener
{
  ...
  private static final String[] QUERIES = {
    "rifugio",
    "fontana", "drinking water", "acqua potabile", "fountain",
    "cascata", "waterfall",
    "parcheggio", "parking",
    "punto panoramico", "viewpoint", "belvedere"
  };
  private static final GoTrailPoiStore.Category[] QUERY_CATEGORIES = {
    GoTrailPoiStore.Category.HUT,
    GoTrailPoiStore.Category.FOUNTAIN, GoTrailPoiStore.Category.FOUNTAIN,
    GoTrailPoiStore.Category.FOUNTAIN, GoTrailPoiStore.Category.FOUNTAIN,
    ...
    GoTrailPoiStore.Category.VIEWPOINT, GoTrailPoiStore.Category.VIEWPOINT,
    GoTrailPoiStore.Category.VIEWPOINT
  };
  ...
  private final Runnable mDotsAnimator = new Runnable()
  {
    @Override public void run()
    {
      ...
      updateActiveStep();
      mDotFrame = (mDotFrame + 1) % 4;
      mUiHandler.postDelayed(this, 180);
    }
  };
  ...
  @Override protected void onCreate(Bundle savedInstanceState)
  {
    ...
    if (getIntent().getBooleanExtra("gotrail_results_only", false))
    {
      ...
      if (cached != null)
      {
        ...
        GoTrailPoiStore.set(cached.lat, cached.lon, cached.items);
        showCachedResults(cached.items);
        return;
      }
    }
  ...
  private void buildUi()
  {
    ...
    final TextView rescan = text("RESCAN", 14, Typeface.BOLD, Color.rgb(11, 95, 215));
    rescan.setGravity(Gravity.CENTER);
    rescan.setPadding(dp(12), dp(7), dp(12), dp(7));
    final GradientDrawable rescanBg = new GradientDrawable();
    rescanBg.setColor(Color.WHITE);
    rescanBg.setStroke(dp(1), Color.rgb(11, 95, 215));
    rescanBg.setCornerRadius(dp(18));
    rescan.setBackground(rescanBg);
    rescan.setOnClickListener(v -> forceRescan());
    radiusRow.addView(rescan, new LinearLayout.LayoutParams(-2, -2));
    ...
    mStepViewpoint = text("Punti panoramici", 19, Typeface.BOLD, Color.DKGRAY);
    ...
    LinearLayout.LayoutParams sfp = new LinearLayout.LayoutParams(-1, -2); sfp.topMargin = dp(8); steps.addView(mStepFountain, sfp);
    ...
    LinearLayout.LayoutParams svp = new LinearLayout.LayoutParams(-1, -2); svp.topMargin = dp(8); steps.addView(mStepViewpoint, svp);
  ...
  private void forceRescan()
  {
    ...
    resetStep(mStepViewpoint, "Punti panoramici");
  ...
  private GoTrailPoiStore.Category classify(@NonNull SearchResult result)
  {
    ...
    if (type.contains("fontana") || type.contains("drinking water") || type.contains("drinking-water") ||
        type.contains("water tap") || type.contains("water point") || type.contains("fountain") ||
        type.contains("acqua potabile")) return GoTrailPoiStore.Category.FOUNTAIN;
    ...
    if (type.contains("punto panoramico") || type.contains("viewpoint") || type.contains("belvedere") ||
        type.contains("scenic") || type.contains("panorama") ||
        name.contains("punto panoramico") || name.contains("viewpoint") ||
        name.contains("belvedere") || name.contains("panorama")) return GoTrailPoiStore.Category.VIEWPOINT;
    return null;
  }
  ...
  private void showCachedResults(@NonNull ArrayList<GoTrailPoiStore.Poi> items)
  {
    ...
    mStepFountain.setVisibility(View.VISIBLE);
    mStepWaterfall.setVisibility(View.VISIBLE);
    mStepParking.setVisibility(View.VISIBLE);
    mStepViewpoint.setVisibility(View.VISIBLE);
  ...
  private void renderResultGroups(@NonNull ArrayList<GoTrailPoiStore.Poi> items)
  {
    ...
    addCategorySection("FONTANE", GoTrailPoiStore.Category.FOUNTAIN, fountains, items);
    ...
    addCategorySection("PUNTI PANORAMICI", GoTrailPoiStore.Category.VIEWPOINT, viewpoints, items);
  }
  ...
  private void finishAnalysis()
  {
    ...
    final ArrayList<GoTrailPoiStore.Poi> items = new ArrayList<>(mFound.values());
    Collections.sort(items, Comparator.comparingDouble(a -> a.distanceKm));
    GoTrailPoiStore.set(mCenterLat, mCenterLon, items);
                                        ...
                                        boolean completed, int count, int activeDot)
  {
    ...
    if (!completed)
    {
      final int dotCount = 4 + Math.abs(activeDot % 4);
      final int travel = Math.abs((activeDot / 2) % 9);
      final StringBuilder spaces = new StringBuilder();
      for (int i = 0; i < travel; i++) spaces.append(' ');
      final StringBuilder db = new StringBuilder();
      for (int i = 0; i < dotCount; i++) db.append('.');
      final String object = "\uFFFC";
      final String raw = marker + "  " + label + "  " + spaces + object + " " + db;
      ...
      {
        final int size = dp(23);
        icon.setBounds(0, 0, size, size);
        out.setSpan(new ImageSpan(icon, ImageSpan.ALIGN_BOTTOM), iconPos, iconPos + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
      }
      final int dotsStart = raw.indexOf(db.toString(), iconPos + 1);
      final int dotsEnd = dotsStart + db.length();
      if (dotsStart >= 0 && dotsEnd > dotsStart)
      {
        out.setSpan(new RelativeSizeSpan(0.88f), dotsStart, dotsEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        out.setSpan(new ForegroundColorSpan(Color.DKGRAY), dotsStart, dotsEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
      }
      return out;
    }
