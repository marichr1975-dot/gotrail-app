...
final class MainSceneDelegate: UIResponder, UIWindowSceneDelegate {
  ...
  func scene(_ scene: UIScene,
    ...
    // GoTr-Ail is the real first screen. Organic Maps is not attached to the
    // window until the user explicitly enters the map.
...
final class GoTrailHomeViewController: UIViewController {
  private let blue = UIColor(red: 11.0 / 255.0, green: 95.0 / 255.0, blue: 215.0 / 255.0, alpha: 1)
  private let green = UIColor(red: 32.0 / 255.0, green: 168.0 / 255.0, blue: 90.0 / 255.0, alpha: 1)
  private let darkText = UIColor(red: 23.0 / 255.0, green: 44.0 / 255.0, blue: 67.0 / 255.0, alpha: 1)
  ...
  private func buildInterface() {
    ...
    shade.colors = [
      ...
      UIColor(red: 0, green: 8.0 / 255.0, blue: 24.0 / 255.0, alpha: 0.34),
      darkText.withAlphaComponent(0.94)
    ]
    view.addSubview(shade)

    // Safe-area based layout: no fixed screen coordinates, so the same code
    // works on SE, standard, Plus/Max and Dynamic-Island devices.
    ...
    let horizontalInset = max(16, min(24, view.bounds.width * 0.055))
    let topInset = max(12, min(28, view.bounds.height * 0.025))
    ...
    NSLayoutConstraint.activate([
      ...
      page.leadingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.leadingAnchor, constant: horizontalInset),
      page.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -horizontalInset),
      page.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: topInset),
      page.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -12)
    ])
  }
  ...
  @objc private func openMap() { GoTrailRootCoordinator.showMap() }
  @objc private func openMaps() { GoTrailRootCoordinator.showMaps() }
  @objc private func openSaved() { GoTrailRootCoordinator.showSaved() }
}
...
final class GoTrailMapHostViewController: UIViewController {
  ...
  @objc private func openSearch() {
    guard let map = MapViewController.shared() else { return }
    map.searchManager.startSearching(isRouting: false)
  }
...
private struct GoTrailCachedPoi: Codable {
  let title: String
  let category: String
  let latitude: Double
...
private struct GoTrailCacheEntry: Codable {
  let timestamp: Date
  let pois: [GoTrailCachedPoi]
}

private final class GoTrailAnalysisCache {
  static let shared = GoTrailAnalysisCache()
  private let key = "GoTr-Ail.AnalysisCache.v32.7"
  private let maxAge: TimeInterval = 30 * 60

  private var storage: [String: GoTrailCacheEntry] = [:]

  private init() {
    guard let data = UserDefaults.standard.data(forKey: key),
          let decoded = try? JSONDecoder().decode([String: GoTrailCacheEntry].self, from: data) else { return }
    storage = decoded
  }

  func load(latitude: Double, longitude: Double) -> [GoTrailCachedPoi]? {
    let entry = storage[cacheKey(latitude: latitude, longitude: longitude)]
    guard let entry, Date().timeIntervalSince(entry.timestamp) < maxAge else { return nil }
    return entry.pois
  }

  func save(latitude: Double, longitude: Double, pois: [GoTrailCachedPoi]) {
    let key = cacheKey(latitude: latitude, longitude: longitude)
    storage[key] = GoTrailCacheEntry(timestamp: Date(), pois: pois)
    if storage.count > 20 {
      storage = storage.sorted { $0.value.timestamp > $1.value.timestamp }
        .prefix(20)
...
private enum GoTrailPoiCategory: String, Codable, CaseIterable {
  case hut = "Rifugi"
...
final class GoTrailAnalyzeViewController: UIViewController {
  ...
  private func buildInterface() {
    view.backgroundColor = UIColor(red: 247.0 / 255.0, green: 249.0 / 255.0, blue: 252.0 / 255.0, alpha: 1)
    ...
    subtitle.textColor = UIColor(red: 95.0 / 255.0, green: 108.0 / 255.0, blue: 121.0 / 255.0, alpha: 1)
    ...
    showMapButton.heightAnchor.constraint(greaterThanOrEqualToConstant: 50).isActive = true
    ...
    NSLayoutConstraint.activate([
      ...
      page.leadingAnchor.constraint(equalTo: scroll.contentLayoutGuide.leadingAnchor, constant: 16),
      page.trailingAnchor.constraint(equalTo: scroll.contentLayoutGuide.trailingAnchor, constant: -16),
      page.topAnchor.constraint(equalTo: scroll.contentLayoutGuide.topAnchor, constant: 16),
      page.bottomAnchor.constraint(equalTo: scroll.contentLayoutGuide.bottomAnchor, constant: -18),
      page.widthAnchor.constraint(equalTo: scroll.frameLayoutGuide.widthAnchor, constant: -32)
    ])
  }
  ...
  private func makeCategoryRow(category: GoTrailPoiCategory) -> UIView {
    ...
    NSLayoutConstraint.activate([
      ...
      label.trailingAnchor.constraint(lessThanOrEqualTo: value.leadingAnchor, constant: -10),
  ...
  private func startAnalysis() {
    ...
    if let cached = GoTrailAnalysisCache.shared.load(latitude: center.latitude, longitude: center.longitude) {
      found = Dictionary(uniqueKeysWithValues: cached.compactMap { cachedPoi in
        guard let category = GoTrailPoiCategory(rawValue: cachedPoi.category) else { return nil }
        let coordinate = CLLocationCoordinate2D(latitude: cachedPoi.latitude, longitude: cachedPoi.longitude)
        let poi = GoTrailIOSPoi(title: cachedPoi.title,
                                category: category,
                                coordinate: coordinate,
                                distanceKm: cachedPoi.distanceKm)
        return (String(format: "%.5f|%.5f|%@", coordinate.latitude, coordinate.longitude, cachedPoi.title.lowercased()), poi)
      })
      setProgress(100)
      status.text = "Analisi disponibile dalla cache locale."
      finishAnalysis(saveCache: false)
      return
    }
  ...
  private func runNextSearch() {
    ...
    searchService?.searchText(item.0, forInputLocale: "it") { [weak self] results in
      ...
      let origin = CLLocation(latitude: self.center.latitude, longitude: self.center.longitude)
      for result in results {
        ...
        guard d <= self.radiusKm else { continue }
        ...
        self.found[key] = GoTrailIOSPoi(title: title, category: item.1, coordinate: c, distanceKm: d)
      }
      self.queryIndex += 1
  ...
  private func finishAnalysis(saveCache: Bool) {
    setProgress(100)
    ...
    if saveCache {
      let cache = ordered.map {
        GoTrailCachedPoi(title: $0.title,
                         category: $0.category.rawValue,
                         latitude: $0.coordinate.latitude,
                         longitude: $0.coordinate.longitude,
                         distanceKm: $0.distanceKm)
      }
      GoTrailAnalysisCache.shared.save(latitude: center.latitude, longitude: center.longitude, pois: cache)
    }
    ...
    for category in GoTrailPoiCategory.allCases {
      ...
      for poi in group {
        let row = UILabel()
        row.numberOfLines = 3
        row.text = String(format: "%@\n%@\n%.1f km dal centro della zona",
                          poi.title, poi.category.rawValue, poi.distanceKm)
